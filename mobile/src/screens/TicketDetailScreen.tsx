...
export const TicketDetailScreen: React.FC<Props> = ({ ticketId, user, onBack, onEdit, onComplete }) => {
  ...
  const takePhoto = async (field?: { checklistId: number; field: ChecklistField }) => {
    if (!ticket) return;
    ...
    setBusy(true);
    try {
      const attachment = await uploadTicketPhoto(ticket.id, result.assets[0].uri);
      if (field) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
          ...
          <Section key={checklist.id} title={checklist.name}>
            {checklist.fields.map((field) => (
              <ChecklistInput
                ...
                onPhoto={() => takePhoto({ checklistId: checklist.id, field })}
              />
            ))}
          </Section>
        ))}
        ...
        <Section title={`Файлы · ${attachments.length}`}>
          ...
          {canModify && <TouchableOpacity style={styles.outlineButton} onPress={() => takePhoto()} disabled={busy}><Text style={styles.outlineText}>Сделать фото</Text></TouchableOpacity>}
        </Section>
        ...
        <Section title={`Комментарии · ${comments.length}`}>
          ...
          {canModify && (
            <View style={styles.commentForm}>
              ...
              <TouchableOpacity style={[styles.send, (!newComment.trim() || busy) && styles.disabled]} onPress={addComment} disabled={!newComment.trim() || busy}><Text style={styles.sendText}>Добавить</Text></TouchableOpacity>
            </View>
          )}
        </Section>
      </ScrollView>
...
const ChecklistInput = ({ field, value, disabled, onChange, onSave, onPhoto }: {
  field: ChecklistField; value: string; disabled: boolean; onChange: (value: string) => void; onSave: (value: string) => void; onPhoto: () => void;
}) => {
  const { colors } = useAppTheme();
  const styles = useMemo(() => createStyles(colors), [colors]);
  if (field.field_type === 'photo') return (
    <View style={styles.checkRow}><View style={styles.checkLabelWrap}><Text style={styles.checkLabel}>{field.label}{field.is_mandatory ? ' *' : ''}</Text>{value ? <Text style={styles.saved}>{value}</Text> : null}</View><TouchableOpacity style={styles.smallButton} onPress={onPhoto} disabled={disabled}><Text style={styles.smallButtonText}>Фото</Text></TouchableOpacity></View>
  );
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  file: { color: colors.secondary, fontSize: 13, paddingVertical: 3 }, outlineButton: { height: 40, borderWidth: 1, borderColor: colors.primary, borderRadius: 7, alignItems: 'center', justifyContent: 'center', marginTop: 9 }, outlineText: { color: colors.primarySoft, fontWeight: '700' },
  comment: { backgroundColor: colors.surface, borderRadius: 7, padding: 10, marginBottom: 7 }, commentHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 5 }, commentAuthor: { color: colors.secondary, fontSize: 11, fontWeight: '700', flex: 1 }, commentDate: { color: colors.subtle, fontSize: 10 }, commentForm: { marginTop: 8 }, commentInput: { minHeight: 70, backgroundColor: colors.input, borderRadius: 7, padding: 10, color: colors.text, textAlignVertical: 'top' }, send: { height: 40, backgroundColor: colors.primary, borderRadius: 7, alignItems: 'center', justifyContent: 'center', marginTop: 7 }, sendText: { color: colors.onPrimary, fontWeight: '700' },
  checkRow: { minHeight: 44, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: colors.border, paddingVertical: 6 }, checkLabelWrap: { flex: 1 }, checkLabel: { color: colors.text, fontSize: 13, flex: 1 }, saved: { color: colors.successBright, fontSize: 10, marginTop: 3 }, checkbox: { width: 24, height: 24, borderWidth: 1, borderColor: colors.subtle, borderRadius: 5, color: colors.onPrimary, textAlign: 'center', lineHeight: 22, marginRight: 9 }, checkboxOn: { backgroundColor: colors.success, borderColor: colors.success }, fieldBlock: { paddingVertical: 7 }, fieldLine: { flexDirection: 'row', gap: 7, marginTop: 5 }, fieldInput: { flex: 1, height: 40, backgroundColor: colors.input, borderRadius: 7, color: colors.text, paddingHorizontal: 10 }, smallButton: { minWidth: 48, height: 36, backgroundColor: colors.primary, borderRadius: 6, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 9 }, smallButtonText: { color: colors.onPrimary, fontSize: 12, fontWeight: '800' },
